...
%y_meas = tcspc_data(:,2)/max(tcspc_data(:,2)); % Load the y_meas data
y_meas =  pl_norm_integrate(1,:);

% Set the initial guess for the parameters to be optimized
params0 = [1e6, 4e-11, 0.004, 1e13, 3, 1000, 1000, 1000];

% Set the options for the non-linear least squares optimization algorithm
options = optimoptions('lsqnonlin', 'Display', 'iter', 'TolFun', 1e8);

% Use lsqnonlin to optimize the parameters to fit the experimental data
params_fit = lsqnonlin(@(params) fit_pdepe(params, t_meas, y_meas), params0, [], [], options);
...
plot(t_meas, y_fit/max(y_fit), '-', 'LineWidth', 2);
xlabel('Time (s)');
set(gca, 'YScale', 'log') %semilog y scale
ylabel('Integrated Carrier Concentration (cm^{-2})');
